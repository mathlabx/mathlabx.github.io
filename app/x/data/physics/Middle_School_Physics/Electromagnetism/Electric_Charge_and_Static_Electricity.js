...
$X.physics.Middle_School_Physics.Electromagnetism.Electric_Charge_and_Static_Electricity = function (charge, staticElectricity) {
    ...
    if (charge) {
        ...
        answers.push(`
        \\text{The interaction between ${object1} and ${object2} is governed by Coulomb's law, which describes the electrostatic force between two charged objects as }
        F = k * |q1 * q2| / r^2,
        \\text{where F is the force, k is Coulomb's constant (8.99 x 10^9 Nm²/C²), q1 and q2 are the charges (${charge1} C and ${charge2} C), and r is the distance (${distance} cm). The force between the objects is approximately ${force} N.}
        `);
